...
int main(int argc, char* argv[]){
    std::string tb_path = "results/mapping_representation_learning/TransMap/";
    int out_dim = std::stoi(argv[6]);
    int n_heads = std::stoi(argv[7]);  
    int mhd_layers = std::stoi(argv[8]);
    double dropout = std::stod(argv[9]);
    tb_path += "outdim_" + std::to_string(out_dim) + "_nheads_" + std::to_string(n_heads) + "_mhdlayers_" + std::to_string(mhd_layers) + "/" +
               "dropout_" + std::to_string(dropout) + "/";  
    TransMapConfig model_config;
    model_config.setOutDim(out_dim);
    model_config.setNumMhdLayers(mhd_layers);
    model_config.setNHeads(n_heads);
    model_config.setDropout(dropout);
    model_config.setNbFeatures(-1); 
    model_config.setLpeDim(16);
    model_config.setEDfgNodeDim(43);
    model_config.setEDfgEdgeDim(46);
    model_config.setEAdgNodeDim(40);
    model_config.setEAdgEdgeDim(48);
    model_config.setStateDim(256);
    model_config.setFfDim(2*out_dim);
    model_config.setHiddenDim(256);
    model_config.setOrthoScaling(0);
    model_config.setOutDimPolicy(16);
    model_config.setActivationFn("relu");

    run_mrl_experiment<TransMapState>( argv,
        tb_path, 
        true,
        EnumModel::TRANSMAP,
       model_config);
