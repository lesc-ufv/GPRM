...
class GeneralPolicyNet(torch.nn.Module):
    def __init__(self,dim_input_state,dim_input_actions,dim_out_actions,dim_out,slope_leaky_relu):
        super(GeneralPolicyNet,self).__init__()
        self.mlp = torch.nn.Sequential(torch.nn.Linear(dim_input_state+dim_out_actions,dim_out),
                                       torch.nn.Linear(dim_out,dim_out))
        self.linear = torch.nn.Linear(dim_input_actions,dim_out_actions)                            
        self.fc = torch.nn.Linear(dim_out,1)
        self.slope_leaky_relu = slope_leaky_relu

    def forward(self, state_vector:torch.Tensor, actions: torch.Tensor):
